...
class MessengerActor constructor(
    ...
    override fun execute(command: MessengerCommand): Observable<MessageEvent.Internal> {
        when (command) {
            ...
            is MessengerCommand.LoadReactions -> {
                return loadMessages.getListOfReactions()
                    .mapEvents(
                        { list ->
                            Log.d("TAGATA", "${list}")
                            MessageEvent.Internal.ReactionsList(command.error, list)

                        },
                        { MessageEvent.Internal.ErrorLoading(command.error) }
                    )
            }
        }
    }
